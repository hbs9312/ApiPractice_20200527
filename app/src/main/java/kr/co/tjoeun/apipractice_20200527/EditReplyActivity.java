...
//                선택한 진영?
                ...
                if (selectedId == -1) {
                    Toast.makeText(mContext, "진영을 선택해 주세요.", Toast.LENGTH_SHORT).show();
                    return;
                }
...
//                진영 미선택 / 내용이 10글자 이내면 등록 거부 처리. (응용문제)
                if (binding.contentEdt.length() < 10) {
                    Toast.makeText(mContext, "10글자 이상 입력해주세요.", Toast.LENGTH_SHORT).show();
                    return;
                }
